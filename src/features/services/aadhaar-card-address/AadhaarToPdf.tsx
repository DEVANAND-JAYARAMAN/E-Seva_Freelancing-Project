  ...
  const handleFieldChange = (name: string, value: string) => {
    setFormData((prev) => {
      ...
      if (errors[name]) {
        let rule = {};
        if (name === "aadhaarNo") {
          rule = { required: true, requiredMessage: "Aadhaar Number is required", pattern: PATTERNS.AADHAAR, patternMessage: "Must be a valid 12-digit Aadhaar number" };
      ...
      <div className="py-16 flex flex-col items-center justify-center text-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-slate-555 mt-2 max-w-md leading-relaxed">
            Your search request for **Adhaar Number to Adhaar PDF Apply** has been registered. The results will be updated soon.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between border-b border-slate-100 dark:border-slate-900/50 pb-4 gap-2">
        <div>
          ...
          <p className="text-xs text-slate-450 dark:text-slate-555 mt-0.5">
            Locate and download your Aadhaar Card PDF using your 12-digit Aadhaar Number
          </p>
        </div>
