...
export function CrmPage() {
  const [customers, setCustomers] = useLocalStorage<Customer[]>("thuruvan_crm_customers", initialCustomers);
      ...
      <section className="flex flex-col gap-8 w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-50 dark:border-slate-900/30 pb-6">
          <div className="space-y-1.5">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed max-w-2xl">
              Track leads, manage system users, oversee distributors and retailers interactions, and build long-lasting business relations.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-900 shadow-sm flex flex-col gap-2">
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Total Customers</span>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-slate-800 dark:text-white">{totalCount}</span>
              <span className="text-xs font-bold text-emerald-500">+100% all-time</span>
            </div>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-900 shadow-sm flex flex-col gap-2">
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Active Customers</span>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-slate-800 dark:text-white">{activeCount}</span>
              <span className="text-xs font-bold text-emerald-500">{((activeCount / (totalCount || 1)) * 100).toFixed(0)}% engagement</span>
            </div>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-900 shadow-sm flex flex-col gap-2">
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Total Retailers</span>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-slate-800 dark:text-white">{retailerCount}</span>
              <span className="text-xs font-bold text-blue-500">Retailer network</span>
            </div>
          </div>
          <div className="p-6 rounded-2xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-900 shadow-sm flex flex-col gap-2">
            <span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Distributors</span>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-black text-slate-800 dark:text-white">{distributorCount}</span>
              <span className="text-xs font-bold text-purple-500">Channel Partners</span>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-900 text-sm">
                {filteredCustomers.length > 0 ? (
                  filteredCustomers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                      <td className="py-4 px-4">
                        <div className="font-extrabold text-slate-800 dark:text-white">{customer.name}</div>
                        <div className="text-xs font-semibold text-[#005c3a] dark:text-emerald-400">{customer.shopName}</div>
                      </td>
                      ...
                      <td className="py-4 px-4">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-extrabold ${
                          ...
                          {customer.status === "Active" ? <CheckCircle size={10} /> : <AlertCircle size={10} />}
                          {customer.status}
                        </span>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-slate-400 dark:text-slate-600 font-semibold">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-900 shadow-xl overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-900 flex items-center justify-between">
              <h3 className="text-lg font-black text-slate-800 dark:text-white">Add New Client / Customer</h3>
              <button
            ...
            <form onSubmit={handleAddCustomer} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase">Customer Name</label>
                  <input
                    type="text"
                    required
                    value={name}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase">Shop Name</label>
                  <input
                    type="text"
                    required
                    value={shopName}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase">Account Type</label>
                  <select
                    value={type}
