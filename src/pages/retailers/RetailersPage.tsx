...
export function RetailersPage() {
  const [retailers, setRetailers] = useLocalStorage<Retailer[]>("thuruvan_retailers_list", initialRetailersList);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedRetailer, setSelectedRetailer] = useState<Retailer | null>(null);

  // Add / Edit submission handler
  const handleFormSubmit = (data: Omit<Retailer, "id" | "createdDate"> & { id?: string }) => {
    if (data.id) {
      // Edit mode
      setRetailers((prev) =>
        prev.map((item) =>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-extrabold text-[#005c3a] dark:text-emerald-400 uppercase tracking-widest flex items-center gap-1">
                Merchant Operations <Sparkles size={10} className="animate-pulse" />
              </span>
            </div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed max-w-2xl">
              Register, manage, status-guard and balance-audit all authorized client merchants and e-seva agents from a single real-time console.
            </p>
          </div>
