...
export default function AdminPage() {
  ...
  async function fetchStatus() {
    try {
      const res = await fetch(`${LAMBDA_URL}/status`, { headers });
  ...
  async function handleStart() {
    ...
    try {
      const res = await fetch(`${LAMBDA_URL}/start`, { method: "POST", headers });
      const data = await res.json();
      setState("running");
      setPublicIP(data.public_ip || "");
  ...
  async function handleStop() {
    ...
    try {
      await fetch(`${LAMBDA_URL}/stop`, { method: "POST", headers });
      setState("stopped");
      setPublicIP("");
  ...
  const stateColor =
    ...
    state === "stopped" ? "text-red-400" : "text-yellow-400";
