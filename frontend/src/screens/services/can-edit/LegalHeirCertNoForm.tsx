...
interface LegalHeirCertNoFormProps {
  onCancel: () => void;
}

export const LegalHeirCertNoForm: React.FC<LegalHeirCertNoFormProps> = ({
  onCancel,
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const requiredFields = [
      ...
      { name: "aadhaarCard", label: "Aadhaar Card (Front & Back)" },
      { name: "canNumber", label: "Can Number" },
      {
        name: "legalHeirCertificateNumber",
        label: "Legal Heir Certificate Number",
      },
    ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between border-b border-slate-100 dark:border-slate-900/50 pb-4 gap-2">
        <div>
          ...
          <p className="text-xs text-slate-450 dark:text-slate-500 mt-0.5">
            Submit required details to apply for Legal Heir Certificate Number
            services
          </p>
        </div>
        <ServicePaymentBadge
          pricingCategoryId="can-edit"
          serviceId="legal-heir-cert-no"
          serviceName="Legal Heir Certificate Number"
          fallback={50}
        />
      </div>
        ...
        <div className="py-16 flex flex-col items-center justify-center text-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 dark:text-slate-555 mt-2 max-w-md leading-relaxed">
              Your request for **Legal Heir Certificate Number** has been
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <InputField
                  ...
                  label="Legal Heir Certificate Number"
                  type="text"
                  placeholder="Enter legal heir certificate number"
                  value={formData.legalHeirCertificateNumber}
                  error={errors.legalHeirCertificateNumber}
                  disabled={isSubmitting}
                  onChange={(val) =>
                    handleFieldChange("legalHeirCertificateNumber", val)
                  }
                />
              </div>
            </div>
