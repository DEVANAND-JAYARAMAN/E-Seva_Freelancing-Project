...
interface FindCanNumberFormProps {
  onCancel: () => void;
}

export const FindCanNumberForm: React.FC<FindCanNumberFormProps> = ({
  onCancel,
}) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between border-b border-slate-100 dark:border-slate-900/50 pb-4 gap-2">
        <div>
          ...
          <p className="text-xs text-slate-450 dark:text-slate-555 mt-0.5">
            Submit required details to apply for Find Can Number services
          </p>
        </div>
        <ServicePaymentBadge
          pricingCategoryId="can-edit"
          serviceId="find-can-number"
          serviceName="Find Can Number"
          fallback={50}
        />
      </div>
        ...
        <div className="py-16 flex flex-col items-center justify-center text-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 dark:text-slate-555 mt-2 max-w-md leading-relaxed">
              Your request for **Find Can Number** has been registered. The
