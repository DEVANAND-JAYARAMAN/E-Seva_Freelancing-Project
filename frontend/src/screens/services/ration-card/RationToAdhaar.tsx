...
interface RationToAdhaarProps {
  onCancel: () => void;
}

export const RationToAdhaar: React.FC<RationToAdhaarProps> = ({ onCancel }) => {
  ...
  const { isForm, startPayment, paymentView } = usePaidServiceFlow({
    serviceId: "ration-to-adhaar",
    serviceName: "Ration Number To Adhaar Number Find",
    pricingCategoryId: "ration-card",
    retailerCharge: 40,
    formData,
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between border-b border-slate-100 dark:border-slate-900/50 pb-4 gap-2">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 dark:text-white">
            Ration Number To Adhaar Number Find
          </h2>
          <p className="text-xs text-slate-450 dark:text-slate-500 mt-0.5">
            Locate your Adhaar details by verifying Ration Card Number
          </p>
        </div>
        <ServicePaymentBadge
          pricingCategoryId="ration-card"
